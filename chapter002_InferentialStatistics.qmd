...
{{< acr H0 >}}:
: Defect rate is $\leq 2\%$ $(p\leq0.02)$

{{< acr Ha >}}:
: Defect rate is $\gt 2\%$ $p \gt 0.02$
...
A **rank** is a position number assigned to an item in a sorted list. 
It tells you where the item stands relative to others.
...
```{r}
#| label: tbl-spear-rank
#| tbl-cap: Example data on ranking items in a list.

rnk_ex <- data.frame(
  name = c("Alice","Bob","Clara","David","Elena"),
...
$R = `r round(cor(spear_dat$temp_c,spear_dat$rod_length_mm),digits = 2)`$
...
spear_dat_calc |> 
  ...
  stat_cor(method = "pearson", label.x = 20, label.y = 125)+
...
#| out-width: 95%
#| fig-cap: The relationship between the production time and the number of defects. 
