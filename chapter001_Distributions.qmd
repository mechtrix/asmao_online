...
$$\mu = E[X] = np$$
$$p = 0.5 \rightarrow \mu = n \times 0.5 = \frac{1}{2}$$
...
#| label: fig-indep
#| out-width: 75%
#| fig-cap: Two random and independent samples drawn from a normal distribution. They do not show any indepedence.

# Generate two independent sets of random data from normal distributions
set.seed(45)
independent_data_x <- rnorm(100, mean = 0, sd = 1)
independent_data_y <- rnorm(100, mean = 0, sd = 1)

# Create a scatterplot to visualize the independence
independence_plot <- ggplot(data.frame(x = independent_data_x, y = independent_data_y), aes(x = x, y = y)) +
  geom_point() +
  labs(title = "Independence of Two Normal Distributions",
       subtitle = "Mean = 0, Standard Deviation = 1 for both X and Y",
       x = "X",
       y = "Y") +
  theme_few()

ggMarginal(independence_plot,type = "histogram")

```

::: {.content-visible when-profile="script"}

In nature and society, many processes involve a large number of random, independent, and additive factors. 
When these factors combine, their individual effects tend to follow a normal distribution, as predicted by the [CLT](#clt). 
This principle is observed in various contexts, such as the behavior of particles in a gas (Brownian motion), the genetics of traits in populations, or the variability in the heights and weights of individuals in a population.
