...
{{< acr SST >}}: $SST = \sum_{i = 1}^n(y_i -\bar{y})$

- How much do the observed $y_i$ values deviate from the mean $\bar{y}$

{{< acr SSR >}}: $SSR = \sum_{i = 1}^n(\hat{y_i} -\bar{y})$

- How much do the predicted $\hat{y_i}$ values deviate from the mean $\bar{y}$

{{< acr SSE >}}: $SSE = \sum_{i = 1}^n(y_i -\hat{y})$

- How much do the observed $y_i$ values deviate from the predicted $\hat{y}$
...
#| label: fig-lm-data
#| out-width: 95%
#| fig-cap: How doe the screw length depend on the rpm of the machine
#| fig-pos: "H"


screw_data <- tibble(
  rpm = c(100, 120, 140, 160, 180, 200, 220, 240, 260, 280),
  length = c(49.8, 49.9, 50.0, 50.2, 50.3, 50.5, 50.8, 51.0, 51.3, 51.5)
)



screw_data %>% 
  ggplot(aes(x = rpm, y = length))+
  geom_point()+
  # geom_smooth(
  #   method = "lm"
  #   )+
...
```{r}
#| label: tbl-lm-param
#| tbl-cap: The parameters of the linear model

lm_model = lm(length ~ rpm, data = screw_data)
...
The $r^2$ is the proportion of total variability explained by the model:
...
The {{< acr CoD >}} ($r^2$), is a statistical measure that assesses the proportion of the variance in the dependent variable that is explained by the independent variable(s) in a regression model. 
...
y_bar <- mean(screw_data$length)
SST <- sum((screw_data$length - y_bar)^2)
SSR <- sum((predict(lm_model) - y_bar)^2)
SSE <- sum((screw_data$length - predict(lm_model))^2)
...
#| label: fig-lm-SST
#| out-width: 95%
#| fig-cap: Visualization of $SST = \sum_{i = 1}^n(y_i -\bar{y})$

# Fit the model
model <- lm(length ~ rpm, data = screw_data)

# Calculate mean, predictions, and errors
screw_data <- screw_data %>%
  mutate(
    y_mean = mean(length),          # Mean of length (for SST)
    y_hat = predict(model),        # Predicted length (for SSR)
    residual = length - y_hat      # Residuals (for SSE)
  )
...
ggplot() +
  ...
  # ) +
  geom_hline(
    yintercept = screw_data$y_mean[1], 
    color = "blue", 
    linetype = "dashed"
  ) +
  geom_segment(
...
#| label: fig-lm-SSR
#| out-width: 95%
#| fig-cap: Visualization of $SSR = \sum_{i = 1}^n(\hat{y_i} -\bar{y})$
...
ggplot() +
  ...
  geom_hline(
    yintercept = screw_data$y_mean[1], color = "blue", linetype = "dashed"
  ) +
  geom_segment(
    data = screw_data,
    aes(
...
#| label: fig-lm-SSE
#| out-width: 95%
#| fig-cap: Visualization of $SSE = \sum_{i = 1}^n(y_i -\hat{y})$
...
##### Putting it all together: $r^2$ as a proportion
...
- \eqref{r2expvar}: If explained variability ($SSR$) is total variability ($SST$): $r^2 = 1$
- \eqref{r2notexpvar}: If variability from model ($SSE$) is $0$: $r^2 = 1$





### Model Checks - Residuals
...
#| out-width: 95%
#| fig-cap: The residuals should be normally distributed.
...
knitr::include_graphics(here::here("chapter003","gradient_descent.png"))
...
A CNC lathe produces screws with a target length of 50 mm. The process engineers suspect that:
...
set.seed(123)
screw_data <- tibble(
  rpm = c(rnorm(15, mean = 1000, sd = 100), rnorm(15, mean = 2000, sd = 100)),
  feed_rate = 0.001 * rpm + rnorm(30, sd = 0.25),  # Reduced noise (sd = 0.1)
  length = 45 + 0.02 * rpm - 10 * feed_rate + rnorm(30, sd = 0.5)
)
...
```{r}
#| label: fig-mlm-eda-feed-rate
#| out-width: 95%
#| fig-cap: EDA on feed rate
...
```{r}
#| label: fig-mlm-eda-length
#| out-width: 95%
#| fig-cap: EDA on length
...
#| label: fig-mlm-eda
#| out-width: 95%
#| fig-cap: Density plots of the machining process

screw_data_lng <- screw_data |> 
  pivot_longer(
    cols = everything(),
    names_to = "variable",
    values_to = "value"
...
::: {.content-visible when-profile="script"}
A short {{< acr EDA >}} of the data for the multiple linear regression is given in @tbl-mlm-eda.
:::

#### Correlation plot

```{r}
#| label: fig-mlm-corrplot
#| out-width: 95%
#| fig-cap: The correlation plot of the variables.
...
mlm <- lm(length ~ rpm+feed_rate ,data = screw_data)
...
screw_data |> 
  pivot_longer(cols = c("rpm","feed_rate")) %>% 
  ggplot(
    aes(
      x = value, 
      y = length
...
```{r}
#| label: fig-mlm-added-noise
#| out-width: 95%
#| fig-cap: What happens if we add some noise? How robust is the model?
#| 
# Add tiny noise to feed_rate
screw_data_noisy <- screw_data %>%
  mutate(feed_rate = feed_rate + rnorm(30, sd = 0.05))

# Refitted model
model_noisy <- lm(length ~ rpm + feed_rate, data = screw_data_noisy)
# summary(model_noisy) |> tbl_regression()
# summary(mlm) |> tbl_regression()

screw_data_total <- bind_rows(
  screw_data |> add_column("source" = "original"),
  screw_data_noisy |> add_column("source" = "noisy")
)

screw_data_total_lng <- screw_data_total |> 
  pivot_longer(cols = c("rpm","feed_rate"),names_to = "variable",values_to = "value") 

screw_data_total_lng |> 
  # filter(source=="noisy") |> 
  ggplot(
    aes(
      x = value,
  ...
  labs(
    title = "Data with a little added noise",
    subtitle = paste0((100-coef(model_noisy)[3]/coef(mlm)[3]*100) |> round(digits = 0),"% change in slope")
  )+
  theme(
    legend.position = "bottom"
  )
...
How much does the variance of the coefficient for `feed_rate` increase because `feed_rate` is correlated with other predictors (`rpm`)?

- $\text{VIF} = 1$: No correlation with other predictors (ideal)
- $\text{VIF} > 1$: Variance is inflated by a factor of {{< acr VIF >}}

##### How much variance is explained?

```{r}
#| label: tbl-mlm-vif
#| tbl-cap: How much variance is explained among the regressors?

vif_model <- lm(rpm~feed_rate, data = screw_data)
...
\text{VIF}_{\text{rpm}} &= \frac{1}{1 - `r vif_model |> glance() |> pull(r.squared)`} \\
\text{VIF}_{\text{rpm}} &= `r car::vif(mlm)[[1]]`
\end{align}
...
#| label: fig-mlm-vifr2
#| out-width: 95%
#| fig-cap: How the VIF changes with increasing multicollinearity


vif_dat <- data.frame(
  r2 = seq(0.01,0.9,0.01)
) |> 
  mutate(
    vif = 1/(1-r2)
  )
...
vif_dat |> 
  ...
  geom_line()+
  geom_hline(
    yintercept = vif_filter$vif,
    linetype = "dashed"
  )+
  geom_vline(
    xintercept = vif_filter$r2,
    linetype = "dashed"
  )+
  scale_x_continuous(
    breaks = pretty
  ...
  labs(
    title = "Critical VIF and r² of parameters",
    x = "r²"
  )
...
```{r}
#| label: tbl-mlm-vif-rules
#| tbl-cap: Rule of thumbs on VIF

vif_guide <- tibble(
  `VIF Range` = c("VIF = 1", "1 < VIF < 5", "5 ≤ VIF < 10", "VIF ≥ 10"),
  Interpretation = c("No correlation",
                     "Moderate correlation",
                     "High correlation",
                     "Extreme multicollinearity"),
  Action = c("Ideal scenario.",
             "Acceptable, but monitor coefficients.",
             "Consider removing predictors.",
             "Avoid! Coefficients are unreliable.")
)
...
vif_table <- vif_guide %>%
  gt() %>%
  tab_header(
    title = md("**Variance Inflation Factor (VIF) Interpretation Guide**"),
    subtitle = md("Guidelines for assessing multicollinearity in regression models")
  ) %>%
  ...
  tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_body(columns = `VIF Range`)
  )

# Print the table (or include in Quarto document)
vif_table
...
1) Remove one predictor:
  - If `rpm` and `feed_rate` are almost identical, keep only one.


2) Combine predictors:
  - Create a new variable, e.g., `speed_feed_ratio = rpm / feed_rate`.
  
3) Design experiments better:
  - Use orthogonal arrays to minimize predictor correlation.

##### New variable

```{r}
#| label: fig-mlm-added_var
#| out-width: 95%
#| fig-cap: Adding a new variable

screw_data_new_var <- screw_data |> 
  mutate(
    speed_feed_ratio = rpm/feed_rate,
    rpm_c = rpm - mean(rpm),
    feed_rate_c = feed_rate - mean(feed_rate)
  )
...
#| label: fig-sw-odds
#| out-width: 75%
#| fig-cap: Never tell him the odds
#| fig-pos: "H"

knitr::include_graphics(here::here("chapter003","sw_odds.png"))
...
$$\text{Odds} = \frac{p}{1-p}$$
For example: $p = 0.8$, odds are $\frac{0.8}{0.2} = 4$ (or "4 to 1")
...
The odds ratio compares the odds of the outcome for two groups differing by $1$ unit in a predictor $X$. 
For a binary predictor (e.g., treatment vs. control), it’s the ratio of odds between the two groups.

For a continuous predictor, the OR represents the multiplicative change in odds for a 1-unit increase in $X$, holding other variables constant.

\begin{align}
OR = e^{\beta_j}
\end{align}

- $\beta_j > 0, \; OR > X$: Higher $X_j$ increases the odds of $Y = 1$
- $\beta_j < 0, \; OR < X$: Higher $X_j$ decreases the odds of $Y = 1$
- $\beta_j = 0, \; OR = X$: No effect

### Example: Coffe vs. productivity
...
productivity_data <- tibble(
  person = 1:6,
  coffee = c(1, 1, 1, 0, 0, 0),
  productive = c(1, 1, 0, 0, 1, 0)
) %>%
  mutate(
    coffee = factor(coffee, labels = c("No", "Yes")),
    productive = factor(productive, labels = c("No", "Yes"))
  )

# Create the gt table
productivity_table <- productivity_data %>%
  select(-person) %>%  # Exclude the 'person' column for clarity
  gt() %>%
  tab_header(
    title = md("**Coffee and Productivity Dataset**"),
    subtitle = md("Hypothetical data to illustrate odds ratio in logistic regression.")
  ) %>%
  fmt_number() %>%
  cols_label(
    coffee = md("**Drank Coffee**"),
    productive = md("**Productive**")
  ) %>%
  tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_column_labels()
...
Odds of being productive if they drank coffee

$$p = \frac{\text{Number of productive with coffee}}{\text{Total with coffee}} = \frac{2}{3}$$

$$Odds = \frac{p}{1-p} = \frac{2/3}{1/3} = 2$$ (or "2 to 1")

### Odds (without coffee) 

Odds of being productive if they did not drink coffee

$$p = \frac{\text{Number of productive without coffee}}{\text{Total without coffee}} = \frac{1}{3}$$

$$Odds = \frac{p}{1-p} = \frac{1/3}{2/3} = 0.5$$ (or "1 to 2")


### Odds ratio

$$OR = \frac{\text{Odds with coffee}}{\text{Odds without coffee}} = \frac{2}{0.5} = 4$$

People who drank coffee have 4 times higher odds of being productive compared to those who did not.
...
ref |> 
 ggplot(
  ...
  geom_line()+
  labs(
    title = bquote(y == frac(1,1+e^-(beta[0]+beta[1]*x)) ~ ";" ~ beta[0] == 0 ~ ";" ~ beta[1] == 1),
    y = bquote(hat(y))
  )+
  scale_y_continuous(
    expand = c(0,0,0.05,0),
...
dataset |> 
 ggplot(
  ...
  geom_line()+
  labs(
    title = bquote(y == frac(1,1+e^-(beta[0]+beta[1]*x)) ~ ";" ~ beta[0] == 0 ~ ";" ~ beta[1] == list(0,...,5)),
    y = bquote(hat(y)),
...
dataset |> 
 ggplot(
  ...
  geom_line()+
  labs(
    title = bquote(y == frac(1,1+e^-(beta[0]+beta[1]*x)) ~ ";" ~ beta[0] == 0 ~ ";" ~ beta[1] == list(-5,...,0)),
    y = bquote(hat(y)),
    linetype = bquote(beta[1] ~":   ")
  )+
 scale_y_continuous(
