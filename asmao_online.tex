...
\item[\phantomsection\label{acronyms_CoD}{CoD}]
Coefficient of Determination
\item[\phantomsection\label{acronyms_DoE}{DoE}]
Design of Experiments
\item[\phantomsection\label{acronyms_ECDF}{ECDF}]
Empirical Cumulative Density Function
\item[\phantomsection\label{acronyms_EDA}{EDA}]
...
\item[\phantomsection\label{acronyms_ME}{ME}]
Margin of Error
...
Standard Error
\item[\phantomsection\label{acronyms_SSE}{SSE}]
Error Sum of Squares
\item[\phantomsection\label{acronyms_SSR}{SSR}]
Regression Sum of Squares
\item[\phantomsection\label{acronyms_SST}{SST}]
Total Sum of Squares
\item[\phantomsection\label{acronyms_VIF}{VIF}]
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter001_Distributions_files/figure-pdf/fig-bn-dist-slides-1.pdf}
...
\subsection{A random variable}\label{a-random-variable}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter001_Distributions_files/figure-pdf/fig-chi-rnd-x-1.pdf}

}

\caption{\label{fig-chi-rnd-x}A random variable}

\end{figure}%

\subsection{That is squared}\label{that-is-squared}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter001_Distributions_files/figure-pdf/fig-chi-rnd-x-square-1.pdf}

}

\caption{\label{fig-chi-rnd-x-square}A random variable}

\end{figure}%

\subsection{\texorpdfstring{Chi\textsuperscript{2}
properties}{Chi2 properties}}\label{chi2-properties}
...
\subsection{\texorpdfstring{Chi\textsuperscript{2}
Applications}{Chi2 Applications}}\label{chi2-applications}
...
\subsection{\texorpdfstring{Chi\textsuperscript{2} The pdf and
dof}{Chi2 The pdf and dof}}\label{chi2-the-pdf-and-dof}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter001_Distributions_files/figure-pdf/fig-chi-pdf-1.pdf}

}

\caption{\label{fig-chi-pdf}The Chi\^{}2}
...
\subsubsection{Prerequisites and
solution}\label{prerequisites-and-solution}
...
\subsection{Emergence}\label{emergence-1}
...
\item
  \(\mu, \sigma\) are unknown, but can be estimated from a sample with
  \(\bar{x},sd^2\)
\item
  if \(\sigma\) would be known, we could use the z-score:
  \(Z=\frac{\bar{x}-\mu}{\sigma/\sqrt{n}}\sim\mathrm{N}(0,1)\)
\item
  but it is not known, so we replace it by \(sd\):
  \(T = \frac{\bar{x}-\mu}{sd/\sqrt{n}}\)
\item
  This is \textbf{not} normally distributed, because \(sd\) depends on
  the sample
\end{itemize}

\subsubsection{The Trick}\label{the-trick}

\begin{itemize}
\tightlist
\item
  Rewrite \(T\) as
  \(T=\frac{\bar{X}-\mu}{\sigma/\sqrt{n}}\frac{\sigma}{sd} = Z \cdot \frac{\sigma}{sd}\)
\end{itemize}
...
\tightlist
\item
  where \(Z\sim\mathrm{N}(0,1)\) (CLT, \(\bar{x}\) is normal)
\item
  \(\frac{\sigma}{sd}\) is a scaled \(\chi^2\) Distribution
\end{enumerate}
...
The t-distribution is the result of combining a normal random variable
with an independent \(\chi^2\) random variable

\subsubsection{The Distribution}\label{the-distribution}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter001_Distributions_files/figure-pdf/fig-t-dist-sl-1.pdf}
...
\subsection{Classroom example: Confidence
interval}\label{classroom-example-confidence-interval}

\begin{figure}[H]

\centering{

\includegraphics[width=0.65\linewidth,height=\textheight,keepaspectratio]{img/catapult.jpg}

}

\caption{\label{fig-catapult}Out catapult we use for shooting}
...
\subsection{Classroom example: two students shoot a
catapult}\label{classroom-example-two-students-shoot-a-catapult}

\begin{figure}[H]

\centering{

\includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio]{img/catapult.jpg}

}

\caption{\label{fig-catapult-fstat}Our catapult we use for shooting}
...
\tightlist
\item
  \(H_0:\) Variances are equal (\(\sigma^2_a = \sigma^2_B\)), \(H_a:\)
  Variances are unequal (\(\sigma^2_a \neq \sigma^2_B\))
\item
  ...
  \tightlist
  \item
    No, because: \(F = \frac{sd_1^2}{sd_2^2} = \frac{4}{1} = 4\) against
    \(F = \frac{sd_1}{sd_2} = \frac{2}{1} = 2\)
  \end{itemize}
\item
  Calculate F Statistic: \(F=\frac{sd^2_A}{sd^2_B}\)
\item
  find p-value for F-Statistic:
  \texttt{pf(F\_stat,\ dof\_A,\ dof\_B),\ 1-pf(F\_stat,\ dof\_A,\ dof\_B)}
\end{itemize}
...
\includegraphics[width=0.75\linewidth,height=\textheight,keepaspectratio]{chapter001/bathtub.png}
...
\includegraphics[width=0.85\linewidth,height=\textheight,keepaspectratio]{chapter001_Distributions_files/figure-pdf/fig-wbll-dist-slides-1.pdf}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{SamplingMethods/Illustration_bootstrap.png}
...
\subsection{Bootstrapping in general}\label{bootstrapping-in-general}
...
\section{\texorpdfstring{\hyperref[acronyms_CI]{Confidence Interval
(CI)}}{Confidence Interval (CI)}}\label{section-5}

A \hyperref[acronyms_CI]{CI} is a statistical concept used to estimate a
range of values within which a population parameter, such as a
population mean or proportion, is likely to fall. It provides a way to
express the uncertainty or variability in our sample data when making
inferences about the population. In other words, it quantifies the level
of confidence we have in our estimate of a population parameter.
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{SamplingMethods/point_estimate_vs_conf_int.png}

}

\caption{\label{fig-ci-lit}Point estimates vs.~confidence intervals
(Ismay and Kim 2019).}
...
\ldots{} with \(L\) and \(U\) being computed from sample data.

A \(95\%\) \hyperref[acronyms_CI]{CI} for the population mean \(\mu_0\)
implies that if we repeated the sampling process infinitely, \(95\%\) of
the computed intervals would contain \(\mu\).

\subsection{\texorpdfstring{Components of a
\hyperref[acronyms_CI]{CI}}{Components of a CI}}\label{components-of-a}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{Point Estimate}: The sample statistic (\(\bar{x},\hat{p}\))
\item
  \textbf{\hyperref[acronyms_SE]{SE}}: Measures the variability of the
  point estimate
\item
  \textbf{Critical Value}: Derived from the sampling distribution
\item
  \textbf{\hyperref[acronyms_ME]{Margin of Error (ME)}}:
...
\subsection{Why Power analysis}\label{why-power-analysis}
...
\item
  What's the probability my experiment will succeed (as in reject
  \(H_0\) when \(H_0\) is false)
\item
  Is my study even worth running given the effect size I expect
\end{itemize}

\subsection{Power defined}\label{power-defined}

\textbf{Power} = Probability of rejecting \(H_0\) when \(H_a\) is true.
...
\subsection{\texorpdfstring{Power and \(\beta\)
error}{Power and \textbackslash beta error}}\label{power-and-beta-error}
...
\subsection{Example: The Coin Toss}\label{example-the-coin-toss}

\begin{figure}[H]

\centering{

\includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio]{chapter002/023_CoinToss.png}
...
For a binomial test, the test statistic is the number of head \(X\)
under \(H_0\), \(X \sim \text{Binomial}(n,0.5)\)

Critical Value

Reject \(H_0\) if \(x\geq c\), where \(c\) is the smallest integer such
that
...
\subsubsection{Power calculations in this
example}\label{power-calculations-in-this-example}

\begin{itemize}
\tightlist
\item
  The \emph{critical value} is the smallest integer where
  \(P(X\geq c)\leq \alpha\)

  \begin{itemize}
  \tightlist
  \item
    For \(n = 10\) and \(\alpha = 0.05\) \texttt{qbinom(0.95,10,0.5)}
    returns \(8\), meaning we would reject \(H_0\) if there are
    \textbf{8 or more heads} in 10 flips
  \end{itemize}
\end{itemize}

\paragraph{\texorpdfstring{Power is the probability of observing
\(X\geq c\) under
\(H_1\)}{Power is the probability of observing X\textbackslash geq c under H\_1}}\label{power-is-the-probability-of-observing-xgeq-c-under-h_1}

\begin{itemize}
\tightlist
\item
  \texttt{pbinom(c-1,n,p\_h1)} computes \(P(X\leq c-1 |H_1)\)
\item
  \texttt{1-pbinom(...)} gives \(P(X\geq c | H_1)\)
\item
  \(n=10\), \(c = 8\) and \(p_{H1} = 0.6\):
  \texttt{1-pbinom(7,10,0.6)\ =} 0.1672898
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pwr}\SpecialCharTok{::}\FunctionTok{pwr.p.test}\NormalTok{(}\AttributeTok{h =}\NormalTok{ pwr}\SpecialCharTok{::}\FunctionTok{ES.h}\NormalTok{(}\AttributeTok{p1 =} \FloatTok{0.6}\NormalTok{, }\AttributeTok{p2 =} \FloatTok{0.50}\NormalTok{),}
...
\subsection{Power vs.~sample size}\label{power-vs.-sample-size}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-pwr-calc-1.pdf}
...
\subsection{Power and the different
parameters}\label{power-and-the-different-parameters}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-pwr-calc-params-1.pdf}
...
After computing statistical power, we must ask: \emph{``How meaningful
is the effect we're powered to detect?''} Effect sizes quantify the
\textbf{magnitude} of a phenomenon---separate from sample size or
significance.

\subsection{What is an effect size?}\label{what-is-an-effect-size}

\begin{itemize}
\tightlist
\item
  A \textbf{standardized} measure of the strength of a relationship or
  difference.
\item
  Answers: \emph{``How large is the effect in the population?''} (not
  just \emph{``Is it nonzero?''}).
\item
  \textbf{Key Property}: Independent of sample size (unlike
  \emph{p}-values).
\end{itemize}

\textbf{Analogy}: Power tells you if your telescope is strong enough to
see a star (\emph{significance}). Effect size tells you how bright the
star is (\emph{meaningfulness}).

\subsection{Types of effect sizes}\label{types-of-effect-sizes}

\begin{table}

\caption{\label{tbl-effectsize-calc}Some parametric and non-parametric
statistical tests.}

\centering{

\caption*{
{\fontsize{20}{25}\selectfont  \textbf{Common Effect Size Metrics in Statistical Analysis}\fontsize{12}{15}\selectfont } \\ 
{\fontsize{14}{17}\selectfont  \emph{Interpretation guidelines and tidyverse functions}\fontsize{12}{15}\selectfont }
} 
\fontsize{12.0pt}{14.0pt}\selectfont
...
\midrule\addlinespace[2.5pt]
{Mean Difference} & {Cohen's \emph{d}} & {Small: 0.2, Medium: 0.5, Large: 0.8} & {\texttt{effectsize::cohen\_d()}} \\ 
{Correlation} & {Pearson's \emph{r}} & {Small: 0.1, Medium: 0.3, Large: 0.5} & {\texttt{correlation::correlation()}} \\ 
{Binary Outcome} & {Odds Ratio (OR) / Risk Ratio (RR)} & {OR = 2: 2x higher odds in group A} & {\texttt{effectsize::odds\_ratio()}} \\ 
{ANOVA (Group Diffs)} & {Eta-squared (η²) / Omega-squared (ω²)} & {Proportion of variance explained} & {\texttt{effectsize::eta\_squared()}} \\ 
{Regression} & {Standardized β (beta)} & {Change in SD units per 1-SD predictor change} & {\texttt{parameters::standardize\_parameters()}} \\ 
\bottomrule
\end{tabular*}
...
\subsection{Computing effect sizes in
R}\label{computing-effect-sizes-in-r}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Simulate data: Treatment vs. Control}
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{123}\NormalTok{)}
\NormalTok{data\_effect }\OtherTok{\textless{}{-}} \FunctionTok{tibble}\NormalTok{(}
  \AttributeTok{group =} \FunctionTok{rep}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\StringTok{"Treatment"}\NormalTok{, }\StringTok{"Control"}\NormalTok{), }\AttributeTok{each =} \DecValTok{50}\NormalTok{),}
  \AttributeTok{score =} \FunctionTok{c}\NormalTok{(}\FunctionTok{rnorm}\NormalTok{(}\DecValTok{50}\NormalTok{, }\AttributeTok{mean =} \DecValTok{85}\NormalTok{, }\AttributeTok{sd =} \DecValTok{10}\NormalTok{), }\FunctionTok{rnorm}\NormalTok{(}\DecValTok{50}\NormalTok{, }\AttributeTok{mean =} \DecValTok{80}\NormalTok{, }\AttributeTok{sd =} \DecValTok{10}\NormalTok{))}
\NormalTok{)}

\CommentTok{\# Compute Cohen\textquotesingle{}s d (correct formula syntax)}
\NormalTok{effectsize}\SpecialCharTok{::}\FunctionTok{cohens\_d}\NormalTok{(score }\SpecialCharTok{\textasciitilde{}}\NormalTok{ group, }\AttributeTok{data =}\NormalTok{ data\_effect) }
\end{Highlighting}
\end{Shaded}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-effectsize-data-1.pdf}

}

\caption{\label{fig-effectsize-data}The visualization of the data}
...
\item
  Old process:
  \(\text{Mean diameter:}\;10.02mm,\; sd = 0.1mm\;(n = 50)\)
\item
  New process:
  \(\text{Mean diameter:}\;10.00mm,\; sd = 0.05mm\;(n = 50)\)
\end{itemize}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-effectsize-simple-example-1.pdf}

}

\caption{\label{fig-effectsize-simple-example}Distribution of the
process data}

\end{figure}%

\subsection{\texorpdfstring{Calculate Cohens \(d\) (Mean
Difference)}{Calculate Cohens d (Mean Difference)}}\label{calculate-cohens-d-mean-difference}
...
\subsection{Ranks}\label{ranks}

A \emph{rank} is a position of a data point when sorted in
ascending/descending order.

Ties are handled using the \emph{average} rank (two 3rd places, both get
ranked \(3.5\))

\begin{itemize}
\tightlist
\item
  Data: \texttt{{[}5,2,8,2,10{]}}
\item
  Sorted: \texttt{{[}2,2,5,8,10{]}}
\item
  Ranks: \texttt{{[}2,2,3,4,5{]}}
\item
  Ranks with ties: \texttt{{[}1.5,1.5,3,4,5{]}}
\end{itemize}

\subsection{Why ranks for non-parametric
tests}\label{why-ranks-for-non-parametric-tests}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Invariance to Monotonic Transformations}: Ranks preserve
  order, so log/root transformations do not change them
\item
  \textbf{Focus on relative Differences}: Tests like Wilcoxon (rank-sum)
  or Kruskal-Wallis compare \emph{ranks distributions} across groups
\item
  \textbf{Exact p-values per Permutation}: Ranks allow exact tests by
  enumerating all possible rank orderings (for small samples)
\end{enumerate}

\subsection{Common Pitfalls and
Misconceptions}\label{common-pitfalls-and-misconceptions}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Loss of Information}: Ranks discard magnitude, a trade-off for
  robustness
\item
  \textbf{Ties Reduce Power}: Many ties weaken tests
\item
  \textbf{Not ``Assumption-Free''}: Nonpaeametric \(\neq\) no
  assumptions (independence, distribution shapes)
\end{enumerate}

\subsubsection{Exercise on ranks}\label{exercise-on-ranks}

\begin{table}

\caption{\label{tbl-ranks-exercise}Reaction times for two groups of
participants}
...
\subsubsection{Sort the data}\label{sort-the-data}

\begin{table}

\caption{\label{tbl-ranks-exercise02}sorted reaction times for two
...
\subsubsection{Assign Ranks}\label{assign-ranks}

\begin{table}

\caption{\label{tbl-ranks-exercise03}sorted reaction times for two
groups of participants}
...
\subsubsection{Sum of ranks}\label{sum-of-ranks}

\begin{table}

\caption{\label{tbl-ranks-exercise04}sorted reaction times for two
groups of participants}
...
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}lrrr}
\toprule
group & sum\_rank & count & mean\_rank \\ 
\midrule\addlinespace[2.5pt]
A & 10 & 4 & 2.5 \\ 
B & 26 & 4 & 6.5 \\ 
\bottomrule
...
\caption{\label{tbl-qq-smpl-dat}10 datapoints for the creation of the
QQ-plot}
...
\subsubsection{Empirical Quantiles}\label{empirical-quantiles}
...
\caption{\label{tbl-qq-smpl-emprcl}The empirical quantiles}
...
\toprule
sample\_data & sample\_data\_emp\_prob \\ 
\midrule\addlinespace[2.5pt]
1 & 0.1 \\ 
2 & 0.3 \\ 
3 & 0.5 \\ 
...
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}rrr}
\toprule
sample\_data & sample\_data\_emp\_prob & quant\_theo \\ 
\midrule\addlinespace[2.5pt]
1 & 0.1 & 0.9736891 \\ 
2 & 0.3 & 2.1708500 \\ 
3 & 0.5 & 3.0000000 \\ 
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-qq-pts-1.pdf}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-qq-abline-1.pdf}
...
\textbf{?@fig-qq-line} the QQ-points are shown together with the
...
\subsubsection{QQ plot with
probabilities}\label{qq-plot-with-probabilities}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-qq-abline-probgrid-1.pdf}

}

\caption{\label{fig-qq-abline-probgrid}A QQ Plot with the assigned
probabilities on the axis}
...
Because it is hard to judge from \textbf{?@fig-qq-line} if the points
are normally distributed, it makes sense to get limits for normally
distributed points. This is shown in Figure~\ref{fig-qq-bands}. The gray
...
\subsubsection{Expanding to non-normal
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-dist-test-data-1.pdf}

}

\caption{\label{fig-dist-test-data}The example data to show all
quantitative distribution tests.}

\end{figure}%

\subsubsection{Kolmogorov Smirnov}\label{kolmogorov-smirnov}

Measures the distance from the \hyperref[acronyms_ECDF]{Empirical
Cumulative Density Function (ECDF)} to the \hyperref[acronyms_CDF]{CDF}.

\hyperref[acronyms_H0]{H0}: The data comes from the assumed
distribution.
...
\caption{\label{tbl-kstest-result}The ks test result}

\centering{

\fontsize{12.0pt}{14.0pt}\selectfont
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}rrll}
\toprule
statistic & p.value & method & alternative \\ 
\midrule\addlinespace[2.5pt]
1 & 0 & Exact one-sample Kolmogorov-Smirnov test & two-sided \\ 
\bottomrule
\end{tabular*}
...
\subsubsection{Summary ks test}\label{summary-ks-test}
...
\item
  Easy to interpret (maximum deviation between empirical and theoretical
  distribution).
\end{itemize}
...
\caption{\label{tbl-adtest-result}The AD test result}
...
\midrule\addlinespace[2.5pt]
0.9818943 & 0.01073587 & Anderson-Darling normality test \\ 
\bottomrule
\end{tabular*}
...
\subsubsection{summary AD test}\label{summary-ad-test}
...
\item
  Generally applicable (not only normal distribution).
\item
  asy to interpret (maximum deviation between empirical and theoretical
  distribution).
\end{itemize}
...
\item
  used for \(n<5000\)
\item
  Test Statistik ranges between \(0,\ldots,1\) with \(1\) being normally
  distributed.
\end{itemize}
...
\subsubsection{Mathemtatical Intuition}\label{mathemtatical-intuition}

\begin{itemize}
\tightlist
\item
  evaluates whether a sample \(X_1,X_2,\ldots,X_n\) comes from a normal
  distribution by comparing the ordered sample values to the expected
  order.
\end{itemize}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Order Statistics: \(X_{(1)} \leq X_{(2)} \leq \ldots \leq X_{(n)}\)
\item
  Expected Normal Order: taken from \(\mathrm{N}(0,1)\) via
  \(m_i=\Phi^{-1}\frac{i}{n+1}\) with \(\Phi\) being the
  \hyperref[acronyms_CDF]{CDF}
\item
  Weights (\(a_i\)): assign weights to \(a_i\) derived from the
  covariance matrix to account for the variability in different parts of
  the distribution (tails vs.~center)
\end{enumerate}

\subsubsection{Connection to linear
Regression}\label{connection-to-linear-regression}
...
\tightlist
\item
  Dependent variable: The ordered sample
  \(X_{(1)} \leq X_{(2)} \leq \ldots \leq X_{(n)}\)
\item
  Independent variable: The expected normal order statistics
  \(m_1,m_2,\ldots,m_n\)
\item
  Weights: The inverse covariance \(\mathrm{V}^{-1}\) accounts for the
  non-independence of order statistics
\end{itemize}

The test statistic \(W\) is then the coefficient of determination
\(r^2\) of this weighted regression.

\[W = r^2 = \frac{\text{Explained Variance}}{\text{Total Variance}}\]

\subsubsection{The test}\label{the-test}

\begin{table}

\caption{\label{tbl-sw-result}The shapiro Wilk test result}
...
statistic & p.value & method \\ 
\midrule\addlinespace[2.5pt]
0.8705227 & 0.01200034 & Shapiro-Wilk normality test \\ 
\bottomrule
\end{tabular*}
...
\subsection{Comparison of test
results}\label{comparison-of-test-results}

\begin{table}

\caption{\label{tbl-quant-norm-test}Comparison of all quantitative test
...
statistic & p.value & method & alternative \\ 
\midrule\addlinespace[2.5pt]
0.8705227 & 0.01200034 & Shapiro-Wilk normality test & NA \\ 
0.9818943 & 0.01073587 & Anderson-Darling normality test & NA \\ 
1.0000000 & 0.00000000 & Exact one-sample Kolmogorov-Smirnov test & two-sided \\ 
\bottomrule
\end{tabular*}
...
\subsection{sample size influence}\label{sample-size-influence}
...
statistic & p.value & method & alternative \\ 
\midrule\addlinespace[2.5pt]
0.8268107 & 1.231603e-17 & Shapiro-Wilk normality test & NA \\ 
23.8428787 & 3.700000e-24 & Anderson-Darling normality test & NA \\ 
0.5000000 & 1.435019e-65 & Asymptotic one-sample Kolmogorov-Smirnov test & two-sided \\ 
\bottomrule
\end{tabular*}
\end{table}
...
Scenario: A semiconductor manufacturer claims that their production line
yields no more than \(2\%\) defective chips (i.e., the defect rate
\(p_0=0.02\) ). To verify this, a quality engineer randomly samples 500
chips and finds 14 defectives. Is there sufficient evidence at
\(\alpha=0.05\) to conclude that the defect rate exceeds the claimed
\(2\%\)?
...
\subsubsection{Define the Hypothesis}\label{define-the-hypothesis}

\begin{description}
\tightlist
\item[{\hyperref[acronyms_H0]{H0}:}]
Defect rate is \textless{} 2 \%
\item[{\hyperref[acronyms_Ha]{Ha}:}]
Defect rate is \textgreater2 \%
\end{description}

This is a \textbf{right-tailed} test

\subsubsection{The z-test for
proportions}\label{the-z-test-for-proportions}

The underlying distribution for the z-test is the \emph{binomial}
distribution.

The test itself relies on the \emph{normal} approximation to the
\emph{binomial}
...
\item
  Sample proportion: The sample proportion \(\hat{p} = \frac{X}{n}\) is
  the estimator for \(p\)
\end{itemize}

\subsubsection{The Bridge to Normality}\label{the-bridge-to-normality}

Acc. to the \hyperref[acronyms_CLT]{CLT} for large \(n\)
...
\tightlist
\item
  Mean: \(\hat{p}\) is the unbiased estimator of \(p\)
\item
  \hyperref[acronyms_SE]{SE}: The \(sd\) of \(\hat{p}\) is
  \(\sqrt{\frac{p(1-p)}{n}}\)
\item
  Normality Condition: \(np\geq10\) and \(n(1-p)\geq 10\)
\end{enumerate}

\subsubsection{The Z-Statistic}\label{the-z-statistic}

Under \(H_0\) assume \(p = p_0\). The Z-Statistic standardizes
\(\hat{p}\) to
...
\subsubsection{Calculate Test Statistic}\label{calculate-test-statistic}
...
\subsubsection{Compute the p-value}\label{compute-the-p-value}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-z-test-1.pdf}

}

\caption{\label{fig-z-test}The z-test statistic}
...
Scenario: A semiconductor manufacturer produces wafers using three
machines (A, B, C). The QA team suspects that defect rates differ across
machines. They collect data over a week:
...
\subsubsection{Define the Hypotheses}\label{define-the-hypotheses}
...
Statistic}\label{calculate-test-statistic-1}
...
A contingency table (also called a cross-tabulation or two-way table) is
a tabular representation of the relationship between two or more
categorical variables. It displays the frequency distribution of
observations across different categories, allowing us to examine
associations, independence, or dependencies between variables.

\subsubsection{Observed Contingency
table}\label{observed-contingency-table}

\begin{table}

\caption{\label{tbl-chi-cont-ob}The tabulated results for the observed
frequencies}
...
\toprule
 & Machine & Defective & Non-Defective & Row Total \\ 
\midrule\addlinespace[2.5pt]
 & A & 45 & 505 & 550 \\ 
 & B & 30 & 520 & 550 \\ 
 & C & 60 & 490 & 550 \\ 
...
\subsubsection{Calculate Expected
Frequencies}\label{calculate-expected-frequencies}
...
\subsubsection{Expected table}\label{expected-table}

\begin{table}

\caption{\label{tbl-chi-cont-ex}The tabulated results for the expected
frequencies}
...
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}lrr}
\toprule
Machine & Defective & Non\_Defective \\ 
\midrule\addlinespace[2.5pt]
A & 45 & 505 \\ 
B & 45 & 505 \\ 
C & 45 & 505 \\ 
...
\subsubsection{The test result}\label{the-test-result}
...
statistic & p.value & method \\ 
\midrule\addlinespace[2.5pt]
10.891 & 0.004 & Pearson's Chi-squared test \\ 
\bottomrule
\end{tabular*}
...
\subsubsection{Quantitative Normality
Check}\label{quantitative-normality-check}
...
\subsubsection{Test result}\label{test-result}
...
\subsection{One sample Wilcoxon test}\label{sec-wilcox-rank-sum}

Scenario:

A manufacturing plant produces bolts with a target diameter of 10.0 mm.
The quality control team measures 7 randomly selected bolts and obtains
the following diameters (in mm): 8, 9.8, 9.9, 10.0, 10.1, 10.2, 15.0

The team wants to determine if the median diameter differs from the
target at a significance level of \(\alpha=0.05\) .
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-wilcox-one-hist-1.pdf}
...
\subsubsection{Signed Ranks}\label{signed-ranks}

\(D_i = X_1 - 10.00\) and Rank \(|D_i|\)

\begin{table}

\caption{\label{tbl-signed-rank}Signed rank calculation}
...
\toprule
Bolt ID & Bolt
diameter & \(D_i\) & \(|D_i|\) & ranked \(D_i\) & signed rank of \(D_i\) \\ 
\midrule\addlinespace[2.5pt]
1 & 8.0 & -2.0 & 2.0 & 4 & -4 \\ 
2 & 9.8 & -0.2 & 0.2 & 3 & -3 \\ 
3 & 9.9 & -0.1 & 0.1 & 2 & -2 \\ 
...
\subsubsection{Sum of signed ranks}\label{sum-of-signed-ranks}

\begin{itemize}
\tightlist
\item
  Sum of \emph{positive ranks}: 10
\item
  Sum of \emph{negative ranks}: 9
\end{itemize}

\subsubsection{Decision at α Level}\label{decision-at-ux3b1-level}
...
The Wilcoxon signed-rank test statistic \(W\) is the smaller of the two
rank sums. Under \(H_0\), the distribution of \(W\) is symmetric, and we
can enumerate all possible outcomes.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Total number of outcomes: Each bolt can have a positive or negative
  outcome (ignoring \(0\)) \(2^7=128\).
\item
  Constructing the Distribution: For each combination, compute rank sum
  for \(\pm\) differences. The critical value is the largest \(W\) such
  that the cumulative probability \(P(W\leq w)\leq\alpha/2\) (two tailed
  test)
\item
  For \(n = 7\) the cumulative probabilities for \(W\) are precomputed
  in tables.
\end{enumerate}

\subsubsection{W value at significance level 5\% and n =
7}\label{w-value-at-significance-level-5-and-n-7}

\hyperref[acronyms_H0]{H0}: The median of differences is \(0\) (or a
certain value like \(10.00mm\))

Possible ranks of differences:

For \(n = 7\) the ranks of the absolute differences are \emph{always}
\(1,2,\ldots,7\) (independent of data, because for \(H_0\) all sign
combinations are of interest)

\[W_{max} = \sum_{k=1}^7 k = 28\]

All possible sign combinations: \(2^7 = 128\). Every combination is one
\textbf{possible realization of \(W\)}

\subsubsection{Calculation of PMF}\label{calculation-of-pmf}
...
\item
  \(W = 1\): exactly the difference with rank 1 is positive (\(7\)
  combinations, because every difference of the \(7\) could be the one
  with rank 1)
\item
  \(W = 2\):
  ...
  \tightlist
  \item
    the difference with rank \(2\) is positive (and all others are
    negative) \textbf{or}
  \item
    the differences with rank \(1\) and rank \(2\) are positive (\(7\)
    combinations)
  \end{itemize}
\end{itemize}

\subsubsection{\texorpdfstring{table for
\(n = 7\)}{table for n = 7}}\label{table-for-n-7}

\begin{table}

\caption{\label{tbl-wilcox-cdf-n7-01}The exact cumulative
probabilities\ldots{}}
...
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}rrr}
\toprule
W & frequency & cumulative probability \\ 
\midrule\addlinespace[2.5pt]
0 & 0.0078125 & 0.0078125 \\ 
1 & 0.0078125 & 0.0156250 \\ 
2 & 0.0156250 & 0.0312500 \\ 
3 & 0.0156250 & 0.0468750 \\ 
4 & 0.0156250 & 0.0625000 \\ 
5 & 0.0234375 & 0.0859375 \\ 
6 & 0.0234375 & 0.1093750 \\ 
7 & 0.0468750 & 0.1562500 \\ 
8 & 0.0312500 & 0.1875000 \\ 
9 & 0.0546875 & 0.2421875 \\ 
10 & 0.0546875 & 0.2968750 \\ 
11 & 0.0625000 & 0.3593750 \\ 
12 & 0.0625000 & 0.4218750 \\ 
13 & 0.0703125 & 0.4921875 \\ 
14 & 0.0781250 & 0.5703125 \\ 
\bottomrule
\end{tabular*}
...
\caption{\label{tbl-wilcox-cdf-n7-02}\ldots{} for the wilcoxon sigend
rank test}
...
W & frequency & cumulative probability \\ 
\midrule\addlinespace[2.5pt]
15 & 0.0390625 & 0.6093750 \\ 
16 & 0.0625000 & 0.6718750 \\ 
17 & 0.0468750 & 0.7187500 \\ 
18 & 0.0546875 & 0.7734375 \\ 
19 & 0.0390625 & 0.8125000 \\ 
20 & 0.0468750 & 0.8593750 \\ 
21 & 0.0468750 & 0.9062500 \\ 
22 & 0.0234375 & 0.9296875 \\ 
23 & 0.0234375 & 0.9531250 \\ 
24 & 0.0156250 & 0.9687500 \\ 
25 & 0.0156250 & 0.9843750 \\ 
26 & 0.0000000 & 0.9843750 \\ 
27 & 0.0078125 & 0.9921875 \\ 
28 & 0.0078125 & 1.0000000 \\ 
\bottomrule
\end{tabular*}
...
\subsubsection{\texorpdfstring{CDF for
\(n = 7\)}{CDF for n = 7}}\label{cdf-for-n-7}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-wilcox-cdf-n7-1.pdf}

}

\caption{\label{fig-wilcox-cdf-n7}The exact cumulative probabilities for
the wilcoxon sigend rank test}

\end{figure}%

\subsubsection{Estimation of W for small
n}\label{estimation-of-w-for-small-n}

\begin{table}

\caption{\label{tbl-wilcox-small-n-1}The critical W values for small n}

\centering{

\caption*{
{\fontsize{14}{17}\selectfont  critical value for the Wilcoxon-signed-rank-test (two-sided, \ensuremath{\alpha}=0.05)\fontsize{12}{15}\selectfont } \\ 
{\fontsize{9}{11}\selectfont  for small sample sizes (n \ensuremath{\leq} 20)\fontsize{12}{15}\selectfont }
} 
\fontsize{12.0pt}{14.0pt}\selectfont
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}rrl}
\toprule
{\bfseries sample size (n)} & critical value (W, two-sided, \ensuremath{\alpha}=0.05) & {\bfseries Interpretation (reject H₀ if...} \\ 
\midrule\addlinespace[2.5pt]
5 & 0 & W \ensuremath{\leq} 0 \\ 
6 & 0 & W \ensuremath{\leq} 0 \\ 
7 & 2 & W \ensuremath{\leq} 2 \\ 
8 & 3 & W \ensuremath{\leq} 3 \\ 
9 & 5 & W \ensuremath{\leq} 5 \\ 
10 & 8 & W \ensuremath{\leq} 8 \\ 
11 & 10 & W \ensuremath{\leq} 10 \\ 
12 & 13 & W \ensuremath{\leq} 13 \\ 
\bottomrule
\end{tabular*}
...
\caption{\label{tbl-wilcox-small-n-2}The critical W values for small n}
...
{\bfseries sample size (n)} & critical value (W, two-sided, \ensuremath{\alpha}=0.05) & {\bfseries Interpretation (reject H₀ if...} \\ 
\midrule\addlinespace[2.5pt]
13 & 17 & W \ensuremath{\leq} 17 \\ 
14 & 21 & W \ensuremath{\leq} 21 \\ 
15 & 25 & W \ensuremath{\leq} 25 \\ 
16 & 30 & W \ensuremath{\leq} 30 \\ 
17 & 35 & W \ensuremath{\leq} 35 \\ 
18 & 41 & W \ensuremath{\leq} 41 \\ 
19 & 47 & W \ensuremath{\leq} 47 \\ 
20 & 53 & W \ensuremath{\leq} 53 \\ 
\bottomrule
\end{tabular*}
...
\subsubsection{The Wilcox PMF}\label{the-wilcox-pmf}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-wilcox-exact-pmf-1.pdf}

}

\caption{\label{fig-wilcox-exact-pmf}The exact PMF for the wilcoxon
sigend rank test}

\end{figure}%

\subsubsection{The Wilcox cumulative
probabilities}\label{the-wilcox-cumulative-probabilities}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-wilcox-cum-prob-1.pdf}

}

\caption{\label{fig-wilcox-cum-prob}The exact PMF for the wilcoxon
sigend rank test}
...
\subsubsection{Sezenario}\label{sezenario}

A manufacturing plant produces three types of components (A, B, C) using
two different assembly lines (Line 1 and Line 2). The quality control
team suspects that the defect rates may differ between the two lines.
They collect data over a week to test whether component type and defect
occurrence are independent of the assembly line used.

\subsubsection{Hypotheses}\label{hypotheses-1}

\hyperref[acronyms_H0]{H0}: Component type and defect occurrence are
independent of the assembly line (no association).

\hyperref[acronyms_Ha]{Ha}: Component type and defect occurrence are
dependent on the assembly line (association exists).
...
\caption{\label{tbl-chi2-indep-data}The contingency table of the
obeservations.}
...
\toprule
Component & Line1\_Defect & Line1\_NoDefect & Line2\_Defect & Line2\_NoDefect \\ 
\midrule\addlinespace[2.5pt]
A & 12 & 188 & 20 & 180 \\ 
B & 8 & 242 & 15 & 235 \\ 
C & 25 & 175 & 10 & 240 \\ 
...
\subsubsection{The test - Component A}\label{the-test---component-a}

\begin{table}

\caption{\label{tbl-chi2-indep-data-comp-A}The test result for Line A}
...
\midrule\addlinespace[2.5pt]
0 & 1 & 1 & Pearson's Chi-squared test \\ 
\bottomrule
\end{tabular*}
...
\subsubsection{The test - Line vs.~Defect Status (Ignoring
Component)}\label{the-test---line-vs.-defect-status-ignoring-component}

\begin{table}

\caption{\label{tbl-chi2-indep-data-comp-overall}The test result for the
overall data}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-chi2-indep-vis-1.pdf}

}

\caption{\label{fig-chi2-indep-vis}Viualitzation of the data}
...
\subsection{Pearson Corrrelation}\label{pearson-corrrelation}

The pearson correlation \(R\) or \(r\) coefficient is a normalized
...
\caption{\label{tbl-example-data-cov}Example data on the calculation of
the covariance for two variables.}

\centering{

\fontsize{13.0pt}{15.0pt}\selectfont
\begin{tabular*}{1\linewidth}{@{\extracolsep{\fill}}cc}
\toprule
...
\subsubsection{Computing means}\label{computing-means}
...
\begin{align}
\bar{X} = \frac{100.2+100.5+100.9+101.3+101.6}{5} = 100.9
\end{align}

\subsubsection{Computing covariance}\label{computing-covariance}

\begin{table}

\caption{\label{tbl-example-data-cov-calc}Hand calculation of the
XY-covariance}

\centering{

\fontsize{11.0pt}{13.0pt}\selectfont
\begin{tabular*}{1\linewidth}{@{\extracolsep{\fill}}ccccc}
\toprule
Temperature in C & Rod Length in mm & \$X\_i - \textbackslash{}bar\{X\}\$ & \$Y\_i - \textbackslash{}bar\{Y\}\$ & \$(X\_i - \textbackslash{}bar\{X\}) \textbackslash{}cdot (Y\_i - \textbackslash{}bar\{Y\})\$ \\ 
\midrule\addlinespace[2.5pt]
20 & 100.2 & -4 & -0.7 & 2.8 \\ 
22 & 100.5 & -2 & -0.4 & 0.8 \\ 
24 & 100.9 & 0 & 0.0 & 0.0 \\ 
...
\subsubsection{Sum the products}\label{sum-the-products}

\begin{align}
\sum(X_i - \bar{X})(Y_i - \bar{Y}) = 2.8 + 0.8 + 0 + 0.8 + 2.8 = 7.2
\end{align}

Compute Covariance

\begin{align}
\mathrm{Cov}(X,Y) = \frac{7.2}{5} = 1.44
\end{align}
...
\tightlist
\item
  \textbf{Covariance} \(= 1.44\;°C\cdot mm\)
\item
  It's \textbf{positive}, meaning:

  \begin{itemize}
  \tightlist
  \item
    When the machine runs hotter, the rods are longer (thermal
    expansion)
  \end{itemize}
\item
  It's \textbf{not standardized}

  \begin{itemize}
  \tightlist
  \item
    The number \(1.44\) is not ``large'' or ``small'' until compared
    with variance or a \textbf{correlation} is computed
  \end{itemize}
\end{itemize}

\subsubsection{detailed Interpretation}\label{detailed-interpretation}

\begin{itemize}
\tightlist
\item
  \((X_i-\bar{X}) \rightarrow\) How different is this temperature from
  average?
\item
  \((Y_i-\bar{Y}) \rightarrow\) How different is this rod length from
  average?
\item
  Multiply them:
  ...
  \tightlist
  \item
    Positive \(x\) Positive \(\rightarrow\) rods are longer at higher
    temp \(\rightarrow\) \(+\) contribution
  \item
    Negative \(x\) Negative \(\rightarrow\) rods are shorter at lower
    temp \(\rightarrow\) \(+\) contribution
  \item
    Different signs \(\rightarrow\) one high, one low \(\rightarrow\)
    \(-\) contribution
  \end{itemize}
\end{itemize}

The average of all contributions is the \textbf{Covariance}

\subsubsection{Correlation equation}\label{correlation-equation}
...
\subsubsection{drive shaft data}\label{drive-shaft-data}
...
We have data on the the \texttt{diameter} vs.~\texttt{rpm} and want to
know, if there is a correlation between the two variables.

\subsubsection{The correlation}\label{the-correlation}
...
\subsubsection{Testing and computing it}\label{testing-and-computing-it}
...
summary}\label{pearson-correlation-coefficient-summary}

\begin{itemize}
\item
  \(2\) continuous variables, measure the strength and direction of
  their linear relationship: Pearson correlation (Pearson 1895)
\item
...
In \textbf{?@fig-drive-shaft-corr-spear} the example data for a drive
...
of both variables in \textbf{?@fig-drive-shaft-spear-qq}.

The \textbf{spearman} correlation coefficient (\(\rho\)) is based on the
pearson correlation, but applied to \textbf{ranked} data

\subsubsection{data with outliers}\label{data-with-outliers}

\begin{table}

\caption{\label{tbl-spear-ex-raw}The data is now subject to outliers.}
...
4 & 26 & 101.0 & normal \\ 
5 & 28 & 101.3 & normal \\ 
6 & 30 & {\cellcolor[HTML]{BEBEBE}{110.0}} & steep increase \\ 
7 & 32 & {\cellcolor[HTML]{BEBEBE}{120.0}} & very steep \\ 
8 & 34 & {\cellcolor[HTML]{BEBEBE}{130.0}} & extreme \\ 
\bottomrule
\end{tabular*}
...
\subsubsection{The ranks of the data}\label{the-ranks-of-the-data}

\begin{table}

\caption{\label{tbl-spear-ex-calc}The steps in calculating the spearman
correlation.}

\centering{

\fontsize{11.0pt}{13.0pt}\selectfont
\begin{tabular*}{1\linewidth}{@{\extracolsep{\fill}}cccccccc}
\toprule
Obersvation & Temperature in C & Rod Length in mm & Comment & Rank \$X\$ & Rank \$Y\$ & \$d = R\_X-R\_Y\$ & \$d\textasciicircum{}2\$ \\ 
\midrule\addlinespace[2.5pt]
...
6 & 30 & {\cellcolor[HTML]{BEBEBE}{110.0}} & steep increase & 6 & 6 & 0 & 0 \\ 
7 & 32 & {\cellcolor[HTML]{BEBEBE}{120.0}} & very steep & 7 & 7 & 0 & 0 \\ 
8 & 34 & {\cellcolor[HTML]{BEBEBE}{130.0}} & extreme & 8 & 8 & 0 & 0 \\ 
\bottomrule
\end{tabular*}
...
\subsubsection{The spearman rho}\label{the-spearman-rho}

\begin{align}
\rho = 1- \frac{6\sum{d^2}}{n(n^2-1)} = 1 - \frac{0}{8(64-1)} = 1
\end{align}

\subsubsection{The data in a plot}\label{the-data-in-a-plot}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-spear-dat-1.pdf}

}

\caption{\label{fig-spear-dat}The data for which the spearman
...
The spearman \(\rho\) calculates the pearson correlation, but between
the \emph{rank} difference of the variables.
...
\bottomrule\noalign{}
\endlastfoot
\(\sum d_i^2\) & Total squared difference in ranks (rank
disagreement) \\
\(6\) & Derived from algebraic simplification of Pearson's r using rank
variance \\
\(n(n^2 - 1)\) & Comes from variance of ranks and number of
comparisons \\
\(1 - \text{fraction}\) & Ensures perfect agreement yields \(\rho = 1\);
increasing \(d_i^2\) lowers \(\rho\) \\
\end{longtable}

\subsection{Comparison on outlier
data}\label{comparison-on-outlier-data}

\begin{table}
\fontsize{12.0pt}{14.0pt}\selectfont
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}rrrrrrll}
\toprule
estimate & statistic & p.value & parameter & conf.low & conf.high & method & alternative \\ 
\midrule\addlinespace[2.5pt]
0.8620862 & 4.166991 & 5.898272e-03 & 6 & 0.4010414 & 0.9746625 & Pearson's product-moment correlation & two.sided \\ 
1.0000000 & 0.000000 & 4.960317e-05 & NA & NA & NA & Spearman's rank correlation rho & two.sided \\ 
\bottomrule
\end{tabular*}
\end{table}

\subsection{Correlation - methodogical
...
\subsection{Plot of the raw data}\label{plot-of-the-raw-data}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-tst-var-1.pdf}
...
\hyperref[acronyms_H0]{H0}: \(\mu_1 = \mu_2\) (two tailed)

\hyperref[acronyms_Ha]{Ha}: \(\mu_1 > \mu_2\) or \(\mu_1 < \mu_2\)
(one-tailed)
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-t-test-oneside-1.pdf}

}

\caption{\label{fig-t-test-oneside}The idea for a one-sided (t-)test}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-t-test-twoside-1.pdf}

}

\caption{\label{fig-t-test-twoside}The idea for a two-sided (t-)test}

\end{figure}%

\subsubsection{The test statistic}\label{the-test-statistic}

The t-statistic quantifies how far the observed difference in means
(\(\bar{x_1}-\bar{x_2}\)) is from \hyperref[acronyms_H0]{H0} in units of
standard error.
...
\item
  Whether the sample variances are equal (pooled variance) or not
  (Welch's t-test)
\end{itemize}

\subsubsection{Standard error for equal
variances}\label{standard-error-for-equal-variances}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-t-tst-two-indep-1.pdf}
...
\subsubsection{Variance Check}\label{variance-check}
...
\subsubsection{Normality Check}\label{normality-check-1}
...
\subsubsection{The data}\label{the-data-3}
...
\subsubsection{Calculation for unequal
variances}\label{calculation-for-unequal-variances}

The \hyperref[acronyms_SE]{SE} of the variances for unequal variances is
calculated:
...
\subsubsection{The test}\label{the-test-1}
...
Based on the Wilcoxon rank sum test (see
Section~\ref{sec-wilcox-rank-sum}).
...
\hyperref[acronyms_H0]{H0}: The two groups have equal medians

\hyperref[acronyms_Ha]{Ha}: The two groups have unequal medians

\subsubsection{The data}\label{the-data-4}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-mannu-tst-data-1.pdf}
...
\subsubsection{Check for Normality}\label{check-for-normality}
...
\subsubsection{The test}\label{the-test-2}
...
\subsubsection{Special characteristics}\label{special-characteristics}

The paired t-test focuses on the differences between paired
observations.

\begin{itemize}
\tightlist
\item
  (\(X_{1i},X_{2i}\)) may be the \(i\)th pair of observations
  (\(i = 1, \ldots, n\))
\item
...
\hyperref[acronyms_H0]{H0}: \(\mu_D = 0\)

\hyperref[acronyms_Ha]{Ha}: \(\mu_D \neq 0\)
...
\subsubsection{Variances}\label{variances-1}
...
\pandocbounded{\includegraphics[keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/unnamed-chunk-18-1.pdf}}

\subsubsection{Visualization}\label{visualization-3}
...
\subsubsection{The test}\label{the-test-3}
...
\subsubsection{Check for Normality}\label{check-for-normality-1}

\pandocbounded{\includegraphics[keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/unnamed-chunk-20-1.pdf}}

\subsubsection{Special characteristic}\label{special-characteristic}
...
\subsubsection{The data}\label{the-data-5}

\pandocbounded{\includegraphics[keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/unnamed-chunk-21-1.pdf}}

\subsubsection{The test}\label{the-test-4}
...
\subsection{Exercise on paired
samples}\label{exercise-on-paired-samples}

A manufacturing plant has 10 machines producing steel rods. After
recalibration, the diameters of rods from each machine are measured
again. The goal is to determine if the recalibration significantly
changed the mean diameter.

\subsubsection{The data}\label{the-data-6}

\begin{table}

\caption{\label{tbl-exercise-paired-data}The data for the paired test
example}
...
\subsubsection{Your plan}\label{your-plan}

How do we go on?

\subsubsection{Solution: Differences}\label{solution-differences}

\begin{table}

\caption{\label{tbl-exercise-paired-data-sol01}The data for the paired
test example}
...
\subsubsection{Solution: Check Normality of
data}\label{solution-check-normality-of-data}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-data-qq-1.pdf}

}

\caption{\label{fig-data-qq}The distribution of the data}

\end{figure}%

\subsubsection{Solution: Check Normality of the
differences}\label{solution-check-normality-of-the-differences}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-D-diff-1.pdf}

}

\caption{\label{fig-D-diff}The distribution of the differences}

\end{figure}%

\subsubsection{The test}\label{the-test-5}

\begin{table}

\caption{\label{tbl-exercise-paired-data-solution-final}The data for the
paired test example}

\centering{

\fontsize{12.0pt}{14.0pt}\selectfont
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}lllrrrrrl}
\toprule
.y. & group1 & group2 & n1 & n2 & statistic & df & p & is\_paired \\ 
\midrule\addlinespace[2.5pt]
diameter & after & before & 10 & 10 & -6.95806860 & 9.00000 & 6.63e-05 & yes \\ 
diameter & after & before & 10 & 10 & -0.06333031 & 17.99979 & 9.50e-01 & no \\ 
\bottomrule
\end{tabular*}
...
\subsubsection{One-way AOVA - basic
idea}\label{one-way-aova---basic-idea}
...
\subsubsection{Check Variances}\label{check-variances}
...
\subsubsection{Where are the variances
equal?}\label{where-are-the-variances-equal}
...
\subsubsection{More tests}\label{more-tests}
...
\subsubsection{The whole game}\label{the-whole-game}
...
\subsubsection{Complete Model}\label{complete-model}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-anova-mdls-comp-1.pdf}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-anova-mdls-red-1.pdf}
...
\subsubsection{SE and MSE}\label{se-and-mse}
...
\subsubsection{SE and MSE comparison}\label{se-and-mse-comparison}
...
\subsubsection{Explained}\label{explained}
...
\subsubsection{Ratio of Variances}\label{ratio-of-variances}
...
\subsubsection{Crosscheck}\label{crosscheck}
...
\subsubsection{final conclusion}\label{final-conclusion}
...
\subsubsection{The data}\label{the-data-7}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-kruskal-raw-dat-1.pdf}
...
\subsubsection{Check Disitribution}\label{check-disitribution}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-kruskal-qq-1.pdf}
...
\subsubsection{The test}\label{the-test-6}
...
\subsubsection{multiple tests}\label{multiple-tests}
...
\subsubsection{The data}\label{the-data-8}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter002_InferentialStatistics_files/figure-pdf/fig-rep-meas-raw-1.pdf}
...
\subsubsection{Check Distributions}\label{check-distributions}
...
\subsubsection{The test}\label{the-test-7}
...
\subsubsection{p-value adjustment}\label{p-value-adjustment}
...
\subsubsection{The test}\label{the-test-8}
...
\begin{align}
SSE = \frac{1}{n} \sum_{i=1}^{n} (y_i - (\beta_1 x_i+\beta_0))^2 \label{rss}
\end{align}

\subsubsection{Error Determination}\label{error-determination}

\hyperref[acronyms_SST]{Total Sum of Squares (SST)}:
\(SST = \sum_{i = 1}^n(y_i -\bar{y})\)

\begin{itemize}
\tightlist
\item
  How much do the observed \(y_i\) values deviate from the mean
  \(\bar{y}\)
\end{itemize}

\hyperref[acronyms_SSR]{Regression Sum of Squares (SSR)}:
\(SSR = \sum_{i = 1}^n(\hat{y_i} -\bar{y})\)

\begin{itemize}
\tightlist
\item
  How much do the predicted \(\hat{y_i}\) values deviate from the mean
  \(\bar{y}\)
\end{itemize}

\hyperref[acronyms_SSE]{Error Sum of Squares (SSE)}:
\(SSE = \sum_{i = 1}^n(y_i -\hat{y})\)

\begin{itemize}
\tightlist
\item
  How much do the observed \(y_i\) values deviate from the predicted
  \(\hat{y}\)
\end{itemize}

\subsubsection{Key Relationship}\label{key-relationship}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter003_Regression_files/figure-pdf/fig-lm-data-1.pdf}

}

\caption{\label{fig-lm-data}How doe the screw length depend on the rpm
of the machine}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter003_Regression_files/figure-pdf/fig-lm-mdl-1.pdf}

}

\caption{\label{fig-lm-mdl}How doe the screw length depend on the rpm of
the machine}

\end{figure}%

\subsection{The model parameters}\label{the-model-parameters}

\begin{table}

\caption{\label{tbl-lm-param}The parameters of the linear model}
...
\midrule\addlinespace[2.5pt]
(Intercept) & 49 & 48, 49 & <0.001 \\ 
rpm & 0.01 & 0.01, 0.01 & <0.001 \\ 
{R\texttwosuperior} & {0.975} & {} & {} \\ 
\bottomrule
\end{tabular*}
\begin{minipage}{\linewidth}
...
\(r^2\)}{Defining r\^{}2}}\label{defining-r2}

The \(r^2\) is the proportion of total variability explained by the
model:

\begin{align}
r^2 = \frac{SSR}{SST} = 1 - \frac{SSE}{SST} \label{r2}
\end{align}
...
\tightlist
\item
  Ranges from \(0\) to \(1\) (or \(0\% \ldots 100\%\))
\item
  Not a test for causality
\item
  Addition of predictors increases \(r^2\)
\end{itemize}

The \hyperref[acronyms_CoD]{Coefficient of Determination (CoD)}
(\(r^2\)), is a statistical measure that assesses the proportion of the
variance in the dependent variable that is explained by the independent
variable(s) in a regression model. It ranges from \(0\) to \(1\), where
\(0\) indicates that the model does not explain any variability, and
\(1\) indicates that the model explains all the variability. In other
words, \(r^2\) provides insight into the goodness of fit of a regression
model, indicating how well the model's predictions match the observed
data.

\subsubsection{\texorpdfstring{Computing
\(r^2\)}{Computing r\^{}2}}\label{computing-r2}

\paragraph{SST}\label{sst}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter003_Regression_files/figure-pdf/fig-lm-SST-1.pdf}

}

\caption{\label{fig-lm-SST}Visualization of
\(SST = \sum_{i = 1}^n(y_i -\bar{y})\)}

\end{figure}%

\paragraph{SSR}\label{ssr}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter003_Regression_files/figure-pdf/fig-lm-SSR-1.pdf}

}

\caption{\label{fig-lm-SSR}Visualization of
\(SSR = \sum_{i = 1}^n(\hat{y_i} -\bar{y})\)}

\end{figure}%

\paragraph{SSE}\label{sse}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter003_Regression_files/figure-pdf/fig-lm-SSE-1.pdf}

}

\caption{\label{fig-lm-SSE}Visualization of
\(SSE = \sum_{i = 1}^n(y_i -\hat{y})\)}

\end{figure}%

\paragraph{\texorpdfstring{Putting it all together: \(r^2\) as a
proportion}{Putting it all together: r\^{}2 as a proportion}}\label{putting-it-all-together-r2-as-a-proportion}

\begin{align}
r^2 &= \frac{SSR}{SST} \label{r2expvar} \\
&= 1- \frac{SSE}{SST} \label{r2notexpvar}
\end{align}

\begin{itemize}
\tightlist
\item
  \eqref{r2expvar}: If explained variability (\(SSR\)) is total
  variability (\(SST\)): \(r^2 = 1\)
\item
  \eqref{r2notexpvar}: If variability from model (\(SSE\)) is \(0\):
  \(r^2 = 1\)
\end{itemize}

\subsection{Model Checks - Residuals}\label{model-checks---residuals}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter003_Regression_files/figure-pdf/fig-lm-resid-qq-1.pdf}

}

\caption{\label{fig-lm-resid-qq}The residuals should be normally
distributed.}
...
\midrule\addlinespace[2.5pt]
(Intercept) & 48.682 & 0.108 & 449.635 & 0.000 \\ 
rpm & 0.010 & 0.001 & 17.833 & 0.000 \\ 
\bottomrule
\end{tabular*}
...
\midrule\addlinespace[2.5pt]
0.975 & 0.972 & 318.028 & 0.000 & 1.000 & 8.000 & 10.000 \\ 
\bottomrule
\end{tabular*}
...
\includegraphics[width=0.7\linewidth,height=\textheight,keepaspectratio]{chapter003/gradient_descent.png}
...
A CNC lathe produces screws with a target length of 50 mm. The process
engineers suspect that:

\begin{itemize}
\tightlist
\item
  Higher RPM increases length due to material expansion from heat.
\item
  Higher feed rate decreases length due to faster material removal.
\end{itemize}
...
\tightlist
\item
  Higher feed rates require higher RPM to maintain surface finish.
\item
  This introduces multicollinearity, complicating the analysis.
\end{itemize}

\subsection{The data}\label{the-data-9}

\subsubsection{rpm}\label{rpm}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter003_Regression_files/figure-pdf/fig-mlm-eda-rpm-1.pdf}

}

\caption{\label{fig-mlm-eda-rpm}EDA on rpm}

\end{figure}%

\subsubsection{feed rate}\label{feed-rate}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter003_Regression_files/figure-pdf/fig-mlm-eda-feed-rate-1.pdf}

}

\caption{\label{fig-mlm-eda-feed-rate}EDA on feed rate}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter003_Regression_files/figure-pdf/fig-mlm-eda-length-1.pdf}

}

\caption{\label{fig-mlm-eda-length}EDA on length}

\end{figure}%

\subsubsection{all in one}\label{all-in-one}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter003_Regression_files/figure-pdf/fig-mlm-eda-1.pdf}

}

\caption{\label{fig-mlm-eda}Density plots of the machining process}

\end{figure}%

A short \hyperref[acronyms_EDA]{Explorative Data Analysis (EDA)} of the
data for the multiple linear regression is given in
\textbf{?@tbl-mlm-eda}.
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter003_Regression_files/figure-pdf/fig-mlm-corrplot-1.pdf}

}

\caption{\label{fig-mlm-corrplot}The correlation plot of the variables.}
...
\begin{align}
\text{length} = \beta_0 + \beta_1 \text{rpm} + \beta_1 \text{feedrate} + \epsilon  \label{mlmmodel}
\end{align}
...
\textbf{Characteristic} & \textbf{Beta} & \textbf{95\% CI} & \textbf{p-value} \\ 
\midrule\addlinespace[2.5pt]
rpm & 0.02 & 0.02, 0.02 & <0.001 \\ 
feed\_rate & -10 & -11, -9.7 & <0.001 \\ 
{R\texttwosuperior} & {0.995} & {} & {} \\ 
Adjusted R\texttwosuperior & 0.995 &  &  \\ 
\bottomrule
\end{tabular*}
\begin{minipage}{\linewidth}
Abbreviation: CI = Confidence Interval\\
...
\subsubsection{\texorpdfstring{\(r^2_{adjusted}\)}{r\^{}2\_\{adjusted\}}}\label{r2_adjusted}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter003_Regression_files/figure-pdf/fig-r2vsr2adj-1.pdf}

}

\caption{\label{fig-r2vsr2adj}The influence of \(k\) (number of
predictors) on \(r^2\) and \(r^2_{adjusted}\).}
...
\subsubsection{Visualizing the model}\label{visualizing-the-model}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter003_Regression_files/figure-pdf/fig-mlm-mdl-1.pdf}
...
\subsubsection{How stable is the model?}\label{how-stable-is-the-model}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter003_Regression_files/figure-pdf/fig-mlm-added-noise-1.pdf}

}

\caption{\label{fig-mlm-added-noise}What happens if we add some noise?
...
How much does the variance of the coefficient for \texttt{feed\_rate}
increase because \texttt{feed\_rate} is correlated with other predictors
(\texttt{rpm})?

\begin{itemize}
\tightlist
\item
  \(\text{VIF} = 1\): No correlation with other predictors (ideal)
\item
  \(\text{VIF} > 1\): Variance is inflated by a factor of
  \hyperref[acronyms_VIF]{Variance Inflation Factor (VIF)}
\end{itemize}

\paragraph{How much variance is
explained?}\label{how-much-variance-is-explained}

\begin{table}

\caption{\label{tbl-mlm-vif}How much variance is explained among the
regressors?}
...
\textbf{Characteristic} & \textbf{Beta} & \textbf{95\% CI} & \textbf{p-value} \\ 
\midrule\addlinespace[2.5pt]
feed\_rate & 901 & 745, 1,056 & <0.001 \\ 
{R\texttwosuperior} & {0.834} & {} & {} \\ 
\bottomrule
\end{tabular*}
\begin{minipage}{\linewidth}
Abbreviation: CI = Confidence Interval\\
...
\paragraph{Computing VIF}\label{computing-vif}

\begin{align}
\text{VIF}_{\text{rpm}} &= \frac{1}{1 - r^2_{\text{rpm ~ feedrate}}} \\
\text{VIF}_{\text{rpm}} &= \frac{1}{1 - 0.834067} \\
\text{VIF}_{\text{rpm}} &= 6.026529
\end{align}

\paragraph{\texorpdfstring{VIF and
\(r^2\)}{VIF and r\^{}2}}\label{vif-and-r2}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter003_Regression_files/figure-pdf/fig-mlm-vifr2-1.pdf}

}

\caption{\label{fig-mlm-vifr2}How the VIF changes with increasing
multicollinearity}

\end{figure}%

\paragraph{Rule of Thumb for VIF}\label{rule-of-thumb-for-vif}

\begin{table}

\caption{\label{tbl-mlm-vif-rules}Rule of thumbs on VIF}

\centering{

\caption*{
{\fontsize{20}{25}\selectfont  \textbf{Variance Inflation Factor (VIF) Interpretation Guide}\fontsize{12}{15}\selectfont } \\ 
{\fontsize{14}{17}\selectfont  Guidelines for assessing multicollinearity in regression models\fontsize{12}{15}\selectfont }
} 
\fontsize{12.0pt}{14.0pt}\selectfont
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}lll}
\toprule
{\bfseries VIF Range} & {\bfseries Interpretation} & {\bfseries Action} \\ 
\midrule\addlinespace[2.5pt]
{\bfseries VIF = 1} & No correlation & Ideal scenario. \\ 
{\bfseries 1 \textless{} VIF \textless{} 5} & Moderate correlation & Acceptable, but monitor coefficients. \\ 
{\bfseries 5 ≤ VIF \textless{} 10} & High correlation & Consider removing predictors. \\ 
{\bfseries VIF ≥ 10} & Extreme multicollinearity & Avoid! Coefficients are unreliable. \\ 
\bottomrule
\end{tabular*}
...
\paragraph{How to fix the model}\label{how-to-fix-the-model}
...
\tightlist
\item
  If \texttt{rpm} and \texttt{feed\_rate} are almost identical, keep
  only one.
\end{itemize}
...
\item
  ...
  \texttt{speed\_feed\_ratio\ =\ rpm\ /\ feed\_rate}.
\end{itemize}
...
\tightlist
\item
  Use orthogonal arrays to minimize predictor correlation.
\end{itemize}

\paragraph{New variable}\label{new-variable}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter003_Regression_files/figure-pdf/fig-mlm-added_var-1.pdf}

}

\caption{\label{fig-mlm-added_var}Adding a new variable}

\end{figure}%

\subsubsection{Residuals}\label{residuals-2}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter003_Regression_files/figure-pdf/fig-mlm-resid-pattern-1.pdf}
...
\subsubsection{Residual Distribution}\label{residual-distribution-1}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter003_Regression_files/figure-pdf/fig-mlm-resid-nd-1.pdf}
...
\paragraph{residuals}\label{residuals-3}
...
\pandocbounded{\includegraphics[keepaspectratio]{chapter003_Regression_files/figure-pdf/unnamed-chunk-12-1.pdf}}
...
\subsubsection{The data}\label{the-data-10}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter005_DoE_files/figure-pdf/fig-ofat-1.pdf}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter005/ANOVA_DoE.png}
...
\hyperref[acronyms_DoE]{Design of Experiments (DoE)}
...
\tightlist
\item
  Factors (X's): Independent variables (e.g., temperature, pressure,
...
\item
  Blocking: Grouping similar experimental units to control nuisance
  variables.
\end{itemize}
...
\tightlist
\item
  Factorial Designs: Study all combinations of factor levels (e.g.,
  \(2^2 = 4 \text{runs}\) for \(2\) factors at \(2\) levels).
\item
  ...
  \tightlist
  \item
    Main effect: Change in response due to one factor (e.g., increasing
    temperature increases yield).
  \item
    Interaction: Effect of one factor depends on another (e.g.,
    temperature and pressure interact to affect yield).
  \end{itemize}
\item
  Orthogonality: Factors are uncorrelated (balanced designs).
\end{itemize}

\section{DoE Types}\label{doe-types}

\subsection{Full Factorial}\label{full-factorial}
...
\item
  Cons: Exponential growth in runs (e.g., 5 factors at 2 levels → 32
  runs).
\end{itemize}
...
\tightlist
\item
  Subset of full factorial runs (e.g., ½ fraction of 2³ = 4 runs instead
  of 8).
\item
  Trade-off: Some interactions are confounded (aliased) with main
  effects.
\item
...
(RSM)}\label{response-surface-methodology-rsm}
...
\item
  Example: Optimizing yield by modeling temperature and pressure with
  quadratic terms.
\end{itemize}

\subsection{Taguchi Methods}\label{taguchi-methods}
...
\section{Steps in a DoE}\label{steps-in-a-doe}

Be bold, but not stupid

\subsection{Define Objectives}\label{define-objectives}
...
\subsection{Select Factors \& Levels:}\label{select-factors-levels}

Use subject-matter knowledge or preliminary experiments.

\subsection{Choose Design}\label{choose-design}

Full/fractional factorial? RSM? Taguchi?

\subsection{Randomize \& Run
Experiments:}\label{randomize-run-experiments}

Avoid bias (e.g., time-order effects).

\subsection{Analyze Data:}\label{analyze-data}

ANOVA, regression, effect plots.

\subsection{Interpret \& Validate:}\label{interpret-validate}
...
\section{Case Study - The catapult}\label{case-study---the-catapult}

\subsection{Measurement System}\label{measurement-system}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter005_DoE_files/figure-pdf/fig-msa-run-chart-1.pdf}

}

\caption{\label{fig-msa-run-chart}The MSA run chart.}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter005_DoE_files/figure-pdf/fig-msa-qq-1.pdf}

}

\caption{\label{fig-msa-qq}qq-plot of results}

\end{figure}%

\subsubsection{possible tolerance
window}\label{possible-tolerance-window}

With a \(C_p = 2\) the tolerance window is \(\pm1.2cm\).

\subsubsection{systematic error}\label{systematic-error}
...
\midrule\addlinespace[2.5pt]
tape\_reading\_cm & 1 & null model & 50 & -9.134823 & 49 & 3.72e-12 \\ 
\bottomrule
\end{tabular*}
\end{table}

With a p-value of \ensuremath{3.72\times 10^{-12}} there is a
...
\subsubsection{check experiment vs.~tolerance
window}\label{check-experiment-vs.-tolerance-window}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter005_DoE_files/figure-pdf/fig-fac-levels-vs-tol-1.pdf}

}

\caption{\label{fig-fac-levels-vs-tol}How good can the factor levels be
measured?}
...
\paragraph{model quality}\label{model-quality}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter005_DoE_files/figure-pdf/fig-model-q-1.pdf}

}

\caption{\label{fig-model-q}Model quality (including repetitions)}

\end{figure}%

\paragraph{significance}\label{significance-1}

\begin{table}

\caption{\label{tbl-aov}ANOVA results (full nmodel)}
...
\toprule
\textbf{Characteristic} & \textbf{Beta} & \textbf{95\% CI} & \textbf{p-value} & \textbf{VIF} \\ 
\midrule\addlinespace[2.5pt]
A\_LaunchAngle & -26 & -29, -22 & <0.001 & 1.0 \\ 
B\_ArmLength & -0.63 & -4.4, 3.1 & 0.7 & 1.0 \\ 
C\_RubberBands & 22 & 18, 26 & <0.001 & 1.0 \\ 
A\_LaunchAngle * B\_ArmLength & 3.6 & -0.16, 7.3 & 0.060 & 1.0 \\ 
A\_LaunchAngle * C\_RubberBands & -15 & -19, -11 & <0.001 & 1.0 \\ 
B\_ArmLength * C\_RubberBands & 0.57 & -3.2, 4.3 & 0.8 & 1.0 \\ 
A\_LaunchAngle * B\_ArmLength * C\_RubberBands & -0.93 & -4.7, 2.8 & 0.6 & 1.0 \\ 
{R\texttwosuperior} & {0.950} & {} & {} & {} \\ 
Adjusted R\texttwosuperior & 0.934 &  &  &  \\ 
\bottomrule
\end{tabular*}
\begin{minipage}{\linewidth}
...
\subsection{DoE - main modeling}\label{doe---main-modeling}

\subsubsection{pareto plot}\label{pareto-plot}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter005_DoE_files/figure-pdf/fig-pareto-1.pdf}

}

\caption{\label{fig-pareto}Pareto plot of parameters}

\end{figure}%

\subsubsection{half normal plot}\label{half-normal-plot}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter005_DoE_files/figure-pdf/fig-half-normal-1.pdf}

}

\caption{\label{fig-half-normal}Half normal plot of parameters}

\end{figure}%

\subsubsection{Main effect plot}\label{main-effect-plot}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter005_DoE_files/figure-pdf/fig-main-effect-1.pdf}

}

\caption{\label{fig-main-effect}Main Effect plot}

\end{figure}%

\subsubsection{Interaction effect plot}\label{interaction-effect-plot}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter005_DoE_files/figure-pdf/fig-interaction-effects-1.pdf}

}

\caption{\label{fig-interaction-effects}Interaction plot}

\end{figure}%

\subsection{DoE - robust design}\label{doe---robust-design}

\subsubsection{Pareto plot}\label{pareto-plot-1}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter005_DoE_files/figure-pdf/fig-sd-pareto-1.pdf}

}

\caption{\label{fig-sd-pareto}Pareto plot for robust design model}

\end{figure}%

\subsubsection{Half-normal plot}\label{half-normal-plot-1}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter005_DoE_files/figure-pdf/fig-sd-half-normal-1.pdf}

}

\caption{\label{fig-sd-half-normal}Half-normal plot for robust design
model}

\end{figure}%

\subsubsection{Main effect plot}\label{main-effect-plot-1}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter005_DoE_files/figure-pdf/fig-sd-main-effect-1.pdf}

}

\caption{\label{fig-sd-main-effect}Main Effect plot}

\end{figure}%

\subsubsection{Interaction effect plot}\label{interaction-effect-plot-1}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter005_DoE_files/figure-pdf/fig-sd-interaction-effects-1.pdf}

}

\caption{\label{fig-sd-interaction-effects}Interaction plot}

\end{figure}%

\subsection{Check for linearity}\label{check-for-linearity}

\subsubsection{Check center points}\label{check-center-points}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter005_DoE_files/figure-pdf/fig-center-distribution-1.pdf}

}

\caption{\label{fig-center-distribution}Interaction plot}

\end{figure}%

\subsubsection{Test for linear model}\label{test-for-linear-model}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter005_DoE_files/figure-pdf/fig-center-dev-1.pdf}

}

\caption{\label{fig-center-dev}Interaction plot}

\end{figure}%

With a p value of \(0.032\) the center points deviate significantly from
the linear model by about \(9cm\).

\subsection{DoE - final model}\label{doe---final-model}

\subsubsection{Parameters - classic
model}\label{parameters---classic-model}

\begin{table}

\caption{\label{tbl-fin-doe-dummy}Final DoE Model for Distance (dummy
model)}
...
\textbf{Characteristic} & \textbf{Beta} & \textbf{95\% CI} & \textbf{p-value} & \textbf{VIF} \\ 
\midrule\addlinespace[2.5pt]
A\_LaunchAngle & -26 & -31, -20 & <0.001 & 1.0 \\ 
C\_RubberBands & 22 & 17, 27 & <0.001 & 1.0 \\ 
A\_LaunchAngle * C\_RubberBands & -15 & -20, -9.6 & 0.001 & 1.0 \\ 
{R\texttwosuperior} & {0.989} & {} & {} & {} \\ 
Adjusted R\texttwosuperior & 0.982 &  &  &  \\ 
\bottomrule
\end{tabular*}
\begin{minipage}{\linewidth}
Abbreviations: CI = Confidence Interval, VIF = Variance Inflation Factor\\
...
\subsubsection{Parameters - robust design
model}\label{parameters---robust-design-model}

\begin{table}

\caption{\label{tbl-fin-sd-dummy}Final DoE Model for robust design
(dummy model)}
...
\textbf{Characteristic} & \textbf{Beta} & \textbf{95\% CI} & \textbf{p-value} & \textbf{VIF} \\ 
\midrule\addlinespace[2.5pt]
A\_LaunchAngle & -2.8 & -4.4, -1.2 & 0.008 & 1.0 \\ 
B\_ArmLength & -0.58 & -2.1, 0.98 & 0.4 & 1.0 \\ 
A\_LaunchAngle * B\_ArmLength & 2.3 & 0.69, 3.8 & 0.016 & 1.0 \\ 
{R\texttwosuperior} & {0.913} & {} & {} & {} \\ 
Adjusted R\texttwosuperior & 0.847 &  &  &  \\ 
\bottomrule
\end{tabular*}
\begin{minipage}{\linewidth}
Abbreviations: CI = Confidence Interval, VIF = Variance Inflation Factor\\
...
\subsubsection{Final Catapult Model}\label{final-catapult-model}

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter005_DoE_files/figure-pdf/fig-fin-grid-vis-1.pdf}

}

\caption{\label{fig-fin-grid-vis}Visualization of final model including
robust design}

\end{figure}%

\subsection{Fractional Factorial (Grömping
2014)}\label{fractional-factorial-gr_mping_2014}

Goal: Identify main effects and key interaction with fewer experiments

\subsubsection{Experimental plan}\label{experimental-plan}

Original:
\(2^3 \text{(factor levels)}* 3 \text{(repetitions)} + 6\text{(center points)} = 30 \text{ experiments}\)

Fractional (minimal):
\(2^{3-1} \text{(factor levels)}* 1 \text{(no repetitions)} + 0\text{(center points)} = 4 \text{ experiments}\)
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{chapter005_DoE_files/figure-pdf/fig-conf-comparison-1.pdf}

}

\caption{\label{fig-conf-comparison}What is confounding?}
...
\caption{\label{tbl-conf}Example calculation to show what confounding
means}
...
\item
  provides a way to classify designs based on how severely factors and
  interactions are confounded
\item
  Higher resolution means less confounding and more reliable estimation
  of effects
  ...
  \item
    IV: Main effects are confounded with three-way (or higher)
    interactions, but two-way interactions are confounded with each
    other
  \item
    V: Main effects and two-way interactions are confounded only with
...
\tightlist
\item
  Lower resolution (e.g., R-III): Cheaper (fewer runs) but riskier
  because main effects may be confounded with two-way interactions.
\end{itemize}

\textbf{If interactions exist, you might misinterpret the results.}

\begin{itemize}
\tightlist
\item
  Higher resolution (e.g., R-V): More expensive (more runs) but provides
  clearer estimates of main effects and two-way interactions.
\end{itemize}

\textbf{Preferred when interactions are suspected or critical.}

\subsubsection{available designs and
resoultion}\label{available-designs-and-resoultion}

\pandocbounded{\includegraphics[keepaspectratio]{chapter005_DoE_files/figure-pdf/unnamed-chunk-6-1.pdf}}
...
\bibitem[\citeproctext]{ref-Gr_mping_2014}
...
Software} 56 (1). \url{https://doi.org/10.18637/jss.v056.i01}.
...
\bibitem[\citeproctext]{ref-statistical_ismaykim_2019}
Ismay, Chester, and Albert Y. Kim. 2019. \emph{Statistical Inference via
...
\bibitem[\citeproctext]{ref-LienigUnknownTitle2017}
